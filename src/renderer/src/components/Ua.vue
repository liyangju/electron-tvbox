        ...
        <ul class="drawer-list">
          <li>
            ...
            <div class="drawer-item-div">
              ...
              <el-button type="success" @click="tokenRefresh" :disabled="tokening">刷新Token</el-button>
            </div> 
          </li>
          <li>
          ...
          <li>
            <el-text class="mx-info" type="info">所有配置数据仅保存本地，刷新完Token后需点击“保存配置”按钮</el-text>
          </li>
        </ul>
      </template>
      <template #footer>
...
const downLocal = async () => {
  ...
  try {
    const result = await window.api.update(url, name, JSON.stringify(config.value))
    console.log(result)
    if (result == 'error') {
      ElMessage.error('下载失败，请检查线路')
    } else {
